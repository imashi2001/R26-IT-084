...
Start the **waste** and **animal** FastAPI services (see repo `services/waste-api` and `services/animal-api`) or point **`MODEL_*_URL`** at deployed URLs.
...
- Variables:
  - **`MODEL_WASTE_URL`**, **`MODEL_ANIMAL_URL`** → public URLs of the deployed model services

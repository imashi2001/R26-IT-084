...
/**
 * Two separately deployed FastAPI microservices on Railway.
 * - waste: services/waste-api  (MobileNetV2 - organic / non-organic)
 * - animal: services/animal-api (YOLOv8n   - dog / cat / monkey / crow)
 */
...
module.exports = {
  ...
  // We always run BOTH models per capture, but DEFAULT_MODEL is kept so the
  // generic /predict?model= override (if a client wants to call only one)
  // still resolves to a sensible service.
  ...
  // Weather + risk engine knobs (rule-based, no ML).
